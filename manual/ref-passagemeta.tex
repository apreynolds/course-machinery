...
kind of which that is true. Not for want of options on the import line: it takes
two. \env{level} says where the passage's natural depth \emph{lands in the
document importing it}, and \env{note} is a gloss that document wants printed
under the heading (section~\ref{sec:prose-documents}). Both are properties of
the importer rather than of the passage, so there is nothing in the block to
inject either from, and emitted lines are always bare. The test is not whether a
key sits at the call site but whether the passage file could know its value ---
and a passage cannot know which document is about to take it, nor what that
document wants said about it.
...
restated the import graph, a passage then being imported by exactly one document.
...
The note here used to say: re-add \env{audience} if a passage is ever imported by
two documents, the situation it was invented for and the one that does not exist.
It exists --- eleven of the fourteen passages are now imported twice or three
times --- and \env{audience} has not come back. The case against it never rested
on the import graph alone: the filter still duplicates the picker and the key
still has no build-time effect. And what a passage in two documents actually
turned out to want was not a tag but per-import prose, which is \env{note} on the
import line (section~\ref{sec:prose-documents}).
