...
\begin{itemize}
  ...
  \item \textbf{A problem file carries what is intrinsically its own}: the
    statement, its hint and solution, and its metadata (topics, type,
    difficulty, source, number). Those belong to the problem forever, wherever
    it is used.
\end{itemize}
...
The recognised keys are \env{topics}, \env{type}, \env{difficulty},
\env{source}, \env{number}, and \env{name}. On the LaTeX side they are validated
but discarded -- a mistyped key (\env{topic} for \env{topics}) errors at compile
time, so typos cannot silently un-index a problem. Before inventing a new type, run
...
\subsection{The fact bank}\label{sec:fact-files}

Facts follow the same pattern, one directory over: \emph{one fact per file},
no preamble, sitting in a topic's \file{facts/} folder. A fact is a bare
snippet -- a definition, a theorem statement, a formula -- on which the
machinery deliberately imposes \emph{no} structure: no environment, no
counter, no hint/solution layers. The surrounding document supplies all the
context; the file is just the content plus its metadata:
...
\textbf{Power Rule.}\quad
For any real constant $n$,
\[ \frac{d}{dx}\,x^{n} = n\,x^{n-1}. \]
\end{manexample}

\cs{FactMeta} is \cs{ProblemMeta}'s little sibling: the same store-only
design (the block typeset nothing above), the same textual reader
(\file{find-facts}, section~\ref{sec:build-and-find}), and therefore the same
two disciplines -- one key per line with braced values (a reflowed block
still compiles but silently vanishes from the script's index), and
\texttt{\#} written \texttt{\char`\\\#}. The schema is smaller: \env{source},
\env{kind}, \env{name}, \env{number}, \env{topics}. A fact has
no \env{type} or \env{difficulty} -- those describe a \emph{problem}'s form
and challenge -- and, as ever, nothing extrinsic. \env{kind} and \env{name}
say \emph{what the fact is} (\texttt{theorem}, \texttt{definition}) and what
to call it, so the catch-all bank can render each snippet as its real
result; \env{kind} joins \texttt{--vocab}, because a typo there costs a box
rather than just splitting a search.

A lecture or formula sheet pulls a fact in with
\cs{importfact}\texttt{[kind=\dots, name=\dots, number=\dots,
number=\dots]\{DIR\}\{FILE\}} (section~\ref{sec:lectures}). \env{kind} names
the CourseBoxes environment to wrap the snippet in and \env{name} titles it,
so one bank of bare snippets renders as the theorems and definitions they
actually are; omit \env{kind} and the fact renders bare, which is what a
dense formula sheet wants -- and for that bare case a document can still print
the name and number by redefining the \cs{ShowFactMeta} hook
(section~\ref{sec:ref-factmeta}). You do not retype any of it:
\texttt{find-facts --import --with kind,name,number} reads the block off
disk and writes the line. The demo bank is \file{derivatives/facts/} in the
course repo.
...
$ find-problems --vocab      # the type/topic vocabulary in use, with counts
...
never run LaTeX. \file{find-facts} answers the same questions about the fact
bank. The full command reference is in section~\ref{sec:build-and-find}.
