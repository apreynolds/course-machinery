...
\env{source}, \env{number}, and \env{desc}. On the LaTeX side they are validated
...
\env{number}, \env{topics}, \env{desc}. A fact has no \env{type} or
\env{difficulty} -- those describe a \emph{problem}'s form and challenge --
and, as ever, nothing extrinsic.

A lecture or formula sheet pulls a fact in with
\cs{importfact}\texttt{[number=\dots, desc=\dots]\{DIR\}\{FILE\}}
(section~\ref{sec:lectures}). By default the imported fact renders bare; a
document that wants the provenance shown -- a formula sheet printing
``\S2.1'' beside each rule, say -- opts in by redefining the
\cs{ShowFactMeta} hook (section~\ref{sec:ref-factmeta}). The demo bank is
\file{derivatives/facts/} in the course repo.
