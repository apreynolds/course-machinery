...
\cs{importproblem}\texttt{[OPTIONS]\{DIR\}\{FILE\}} wraps the \file{import}
package's \cs{import} with per-import settings (keyval family
\env{eximport}); \cs{importcheckin} is the same with the check-in flavour
pre-set. Bare \cs{import} still works and yields no marker.
...
\cs{ifsolutions} / \cs{ifhints} drive the two layers; they default from
\cs{ShowSolutions} / \cs{ShowHints}, which build-pdfs injects per view
(section~\ref{sec:build-and-find}). Hint visibility is three-state: the
per-import keys override the document toggle.
