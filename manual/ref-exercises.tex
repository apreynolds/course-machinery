...
\env{problem} & a numbered, cross-referenceable problem (flat document-wide
  counter; \cs{counterwithin} to restart per section). What a \emph{bank file}
  contains: extrinsic settings come from the \cs{importproblem} that pulls it
  in.\\
\env{notesproblem}\texttt{[...]} / \env{notescheckin}\texttt{[...]} & the same
  problem written \emph{in place} in a document rather than imported --- the
  \env{eximport} keys below, with the body where the \cs{import} would be, and
  an identical rendering. For drafting a lecture whole and extracting later. A
  bank file still holds bare \env{problem}; putting a \env{notesproblem} there
  would fight the call site for the same registers.\\
\env{problemgroup}\texttt{[name=\{...\}]\{STEM\}} & assembly-time grouping:
  ``Problem $N$.''\ $+$ stem, members render as parts (a), (b). Label the group
  \emph{inside} it, before the first part. The optional \texttt{name=} is a
  short content label for the group as a whole -- the stem is a sentence, so
  the class lists the name instead, and only when one is given.\\
...
\env{studentonly} & printed in the student copy; in the key its \emph{space}
  is kept and its ink dropped, so a solution above it paints into the room
  (an empty grid to sketch on). Put the \env{solution} \emph{before} it.
  Nothing under \cs{collapseworkspace} (section~\ref{sec:overlay}).\\
...
\env{problemchoice}\texttt{[select=, points=, instruction=]} &
  choose-$N$-of-$M$: counts select$\times$points once.
  \texttt{instruction=each} (default) repeats the generated sentence above
  every member, \texttt{first} prints it once, \texttt{none} never prints it
  (the paper must say it elsewhere).\\
\end{tabularx}
\end{center}
...
\texttt{name=\{...\}} & short content label: a parenthetical after the
  heading, the check-in box's title, and the contents entry (notes).\\
\texttt{label=\{...\}} & notes: the \emph{noun} for this one import, overriding
  the course's \cs{NotesProblemLabelWord} --- \texttt{label=\{Checkpoint\}} in a
  lecture whose other items are Examples. \texttt{label} is what it is
  \emph{called}, \texttt{name} what it is \emph{about}. Reaches the heading, the
  contents entry and a group stem alike, all three going through
  \cs{ProblemLabelWord}.\\
...
\texttt{yourturn} & notes: mark this one as the \emph{student's} to work, not
  yours to present. Prefixes the heading with \cs{YourTurnMark} (CourseBoxes',
  shared with the aside of the same name) and adds the pen glyph to the
  contents entry. Top-level problems only --- on a check-in or a
  \env{problemgroup} member the heading is never rendered, and it warns.\\
\end{tabularx}
\end{center}

Keys a context ignores are tolerated by design -- \texttt{points=} under
LectureNotes, \texttt{number=}/\texttt{name=} under Assessment -- so one
