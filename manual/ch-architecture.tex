...
\subsection{Where the scaffolding still shows}

One piece of scaffolding from the course this was built in is still
visible in normal use: \file{build-pdfs} writes its auxiliary files under
\file{~/.cache/latexmk}, a location it does not ask about and you cannot
configure. Everything still works anywhere -- the directory is created on
demand, and the path is keyed by each source's repo-relative path so two
topics' same-named subdirectories never share latexmk state -- but the
root is fixed rather than derived.

Outstanding work on the toolkit, this item included, is listed in
\file{TODO.md} at the root of the repository. It is kept there rather
than here so that this manual can stay a description of what the toolkit
\emph{does}.
