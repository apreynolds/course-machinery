...
\subsubsection*{The text layer: .textbook/}
...
own \textbf{text layer}, \file{.textbook/textbook.tex}.
...
No master \cs{input}s it. \file{.latexmkrc} puts \file{.textbook/} on
...
whole mechanism rests on \file{.textbook/} being on \env{TEXINPUTS}, and
naming the folder rather than the course root is what keeps that exposure
narrow: only \file{.textbook/*.tex} becomes findable by bare name, so
nothing else in the course can be picked up by accident. The folder also
stops being an oddity as soon as a course has two books, since then it
holds \file{apex.tex} and \file{openstax.tex} side by side.
