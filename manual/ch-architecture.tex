...
\SetTextBase{https://opentext.uleth.ca/apex-calculus/}
\DeclareSection{2.1}{sec_derivative.html}
\DeclareSection{2.2}{sec_common_der.html}
\end{manexsource}

Masters \cs{input} it in the preamble (\cs{newtheorem} must run before
the document starts), by relative path:
\file{\textbackslash input\{../../.textbook/textbook\}}. The
\cs{ifdefined} guard is what lets \emph{one} file serve every master: an
Assessment document loads no CourseBoxes, skips the factory calls, and
still picks up non-box overrides like the notes noun. The manifest lines
need no guard -- all three classes load TextRef
(section~\ref{sec:ref-textref}).

\textbf{Why a folder for one file.} The folder is the unit that travels.
Teach three courses out of the same book and \file{.textbook/} is exactly
what you would promote to a shared submodule (\file{.apex/}) mounted in
all three: the text's vocabulary and link map then live \emph{once}, and
a publisher moving domains is one edit for every course that uses it.
Nothing depends on which it is -- plain folder today, submodule tomorrow,
and no master changes either way. The single \cs{input} is the same
insulation: should the section manifest ever become machine-generated
(scraped from the book's contents, so a regeneration cannot clobber
hand-written vocabulary), \file{textbook.tex} becomes a two-line
aggregator and no document notices.

Promoting it also frees the \emph{name}. \file{.textbook/} is the neutral
choice, and the right one while a course has a single book; a course that
mounts a submodule may instead name the folder for the text it holds
(\file{.apex/apex.tex}), so the tree says at a glance which book the
course teaches out of and each master's \cs{input} line names the text it
is citing. Nothing in the machinery reads the path -- masters \cs{input}
it by relative path and that is the whole coupling -- so this is purely
whether you want the name to carry the information.

Naming earns its keep once a course needs \emph{two}: an opening review
chapter drawn from one book, the rest from another. Mount one layer per
book; two named folders sit side by side without ambiguity, and a
transition year that runs both can do so without either shadowing the
other. The constraint that comes with it is that a given master
\cs{input}s exactly \emph{one} layer, which is worth reading before you
try it (section~\ref{sec:ref-textref}).
