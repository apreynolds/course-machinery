...
in step with the lectures. What remains here is the \emph{base} those
pages hang off, plus the handful of sections the course
\emph{cross-references} -- points at without being. That is why the list
above is one line rather than ninety.
...
review chapter out of one text and the rest out of another: keep a file
per book (\file{textbook.tex} and \file{openstax.tex} in the demo), and
each lecture names the book it means with \cs{usetext}. Two books whose
section numbers collide -- and they will, every book has a \S1.1 -- cannot
be confused, because the book is part of what the lecture declares. Note
that the cross-reference list goes in each \emph{text} file: a lecture
built on one book never loads the other's
...
text layer that was once its only occupant.
