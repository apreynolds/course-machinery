...
\begin{itemize}
  ...
  \item \texttt{-{}-import} prints the matches as paste-ready
    \cs{importproblem} lines. Two opt-in modifiers refine the output:
    \texttt{-{}-with LIST} injects a comma subset of \env{number,desc} as
    options where a file carries them (\env{points} stays manual -- it is
    extrinsic), and \texttt{-{}-from DIR} rewrites each line's directory
...
\subsection{Picking problems interactively}

\file{pick-problems} threads those two machine modes through
...
\texttt{-{}-import} so the accepted set comes out as \cs{importproblem} lines on
...
multiple-choice derivatives/}. The Vim command \texttt{:ImportProblem}
(\texttt{<Space>ip}) is the same pipeline, inserting the lines at the cursor
