...
\subsubsection{Cover page: one page of one PDF}

A second key lives in the same leading block:

\begin{manexsource}
%! coverpage: yes
\end{manexsource}

\noindent That writes \file{FILE_COVERPAGE.pdf} as well --- page~1 of the
finished PDF on its own, for printing an assessment's cover separately from
the paper behind it (section~\ref{sec:assessments} has the assessment-side
account). \texttt{no}, or no line at all, is off; any other value warns.

Unlike a view, this is not a compile: it is a page lifted out of the base
PDF \emph{after} it is built, with ghostscript. That is deliberate rather
than incidental --- a cover carries \cs{printtotalpoints} and ``Page 1 of
$M$'', both two-pass values that only exist once a run has reached
\cs{end\{document\}} and written the \file{.aux}, so a package that selects
page~1 \emph{during} the run prints the point total wrong. It is taken from
the base build only, never a view, and follows the same mirror and
\texttt{BOTH} handling as that PDF. Without ghostscript on the machine the
extract is skipped with a warning and the build still succeeds.
...
\paragraph{Sending a PDF to a named folder.}
Sometimes one PDF belongs somewhere else too -- the instructor copy of a test in
a folder shared with TAs, say, while the rest of the build still mirrors to your
private cloud. Add \texttt{LABEL: path} lines to \file{.MIRRORDIR}, then name the
label from the source in a \texttt{\%! copies:} magic comment of its own:

\begin{manexsource}
%! views: solutions, instructor
%! copies: instructor-->TA-dir
\end{manexsource}
...
the git-ignored \file{.MIRRORDIR} maps it to a path, no machine-specific path
ever enters a tracked source. \env{default} names the base (student) PDF, so
\texttt{default-->LABEL} sends it the same way.

Two lines, not one, because they answer different questions.
\texttt{\%! views:} says which PDFs a document \emph{has};
\texttt{\%! copies:} says where any of them additionally \emph{goes}. A view
named on the copies line must also be requested on the views line -- declaring a
copy never adds a build -- and the arrow is deliberate: its left side is a view
name, its right a label, so nothing has to be guessed. (An older source may
still carry the destination as a \texttt{:LABEL} suffix on the views line. It
builds the view and warns you where the copy moved to.)

\paragraph{Holding a copy until a date.}
Where a PDF goes and \emph{when} it may go are also different questions, and the
second one has teeth: solutions that reach TAs before the assessment cannot be
recalled from a folder other people sync. Give the destination a date:

\begin{manexsource}
%! views: solutions
%! copies: solutions-->TA-dir(from 2026-10-01)
\end{manexsource}

\noindent The destination is inert before that date and an ordinary destination
on and after it. \texttt{from} includes its own day, and the boundary is
midnight, so for a test written on the first you want \texttt{from 2026-10-02}.

That one rule is read twice, and it needs both readings. The build honours it,
so no rebuild before the date can leak the PDF, and it announces a held copy
instead of passing over it in silence. But a build-time check alone would only
ever \emph{suppress}: nothing rebuilds a document that is finished, so an
October copy would sit unsent for ever while looking automatic. So every run
also ends by sweeping the whole repo for copies that have come due, and
\texttt{build-pdfs -{}-release} does that sweep on its own for a scheduler to
call. Building \emph{any} document therefore releases \emph{every} document that
is due.

The sweep keeps no record of what it has done -- it re-reads the sources every
time and sends only what is absent or out of date at the destination. So it
costs nothing to run often, and deleting a \texttt{\%! copies:} line is how you
cancel a standing order.

Everything here fails closed. A malformed entry, a date that is not a real date,
a view the source does not build, a label that does not resolve, a PDF that has
come due but was never built -- each one warns and sends nothing. Late is a
warning you can act on; early is not.
...
$ find-facts    [--topic PAT] [--source PAT] [--untagged] [--vocab]
                [--import] [--tsv] [PATH ...]
$ find-passages [--import] [--tsv] PATH ...
\end{manexsource}

Reads the \cs{ProblemMeta} (resp.\ \cs{FactMeta}) block off every \file{.tex}
file under each \env{PATH} (default: the current directory, recursively) and
reports the matches -- without running LaTeX, which is the point
(section~\ref{sec:problem-files}). With no filters, it lists every file that
has a block. The kind fixes the block macro it reads, the
\cs{importproblem}~/ \cs{importfact} lines it emits, and the filter set --
facts carry no \env{type} or \env{difficulty}, so the two have four filters and
two.

\textbf{\file{find-passages} is the odd one, and differs in kind rather than in
degree.} A passage bank has \emph{no metadata block at all}: the directory is
the bank, every \file{.tex} in it is a passage, and a passage's title and label
are derived from its file name (section~\ref{sec:ch5-passages}). So there is
nothing to search and nothing to inject, and four things the other two accept
are refused outright rather than left silently useless: every filter,
\texttt{-{}-with}, \texttt{-{}-untagged} (it inverts a membership that no
longer exists) and \texttt{-{}-vocab} (there are no tags).

\textbf{It also requires a \env{PATH}}, where the other two default to the
current directory. The bank is a sibling submodule rather than a directory of
the course, so name it --- \texttt{find-passages PASSAGES/passages}. With no
block to tell a passage from any other \file{.tex}, a bare run from a course
root would report every lecture and assessment as a passage, so it is refused.

Throughout this manual \texttt{PASSAGES} stands for whatever your course checks
the passage submodule out as, and \texttt{SYLLABUS} for the directory you are
importing into. Neither can be written concretely here: both are the course's
own choice, and courses differ.

What it emits is a heading and an import, the shape a call site writes by hand:

\begin{manexsource}
$ find-passages --import --from SYLLABUS PASSAGES/passages
\section{Webwork Tips}\label{sec:webwork_tips}

\import{../PASSAGES/passages/}{webwork-tips.tex}
\end{manexsource}

\noindent The label is exact; the title is mechanical and worth reading before
keeping (\file{well-written-solutions.tex} yields ``Well Written Solutions'').
Delete the \cs{section} line when the passage lands under a heading that
already exists.

Everything below holds for all three unless it says otherwise.
...
\cs{importfact}, or a \cs{section} and \cs{import} pair) on
...
\env{points}, and picking a passage drops in a heading and an import.

Two things about picking \emph{passages} in particular. \textbf{Give the bank as
the operand} --- \texttt{pick-passages PASSAGES/passages} --- because a passage
bank has no metadata block and a bare run is refused rather than offering every
\file{.tex} in the course; \texttt{<Space>ig} does this for you. Second, the
blocks come out separated by a blank line, which makes each one two paragraph
objects. That matters because fzf returns a multi-selection in \emph{input}
order, not the order you ticked them, while a passage's position in a document
can be load-bearing wherever one passage refers to another --- so reorder after
pasting, and expect to edit the derived title.
