...
\file{find-problems} searches the problem bank and \file{find-facts} the fact
bank -- each a two-line shim that hands the right \texttt{-{}-kind} to
\file{find-meta}, where all the real logic lives once -- and their
interactive twins \file{pick-problems} / \file{pick-facts} (shims over
\file{pick-meta} the same way) thread those searches through fzf. Every one
of them prints its full reference with \texttt{-{}-help}.
...
\subsection{\texorpdfstring{\texttt{find-problems} /
  \texttt{find-facts}}{find-problems / find-facts}}
...
Reads the \cs{ProblemMeta} (resp.\ \cs{FactMeta}) block off every
\file{.tex} file under each \env{PATH} (default: the current directory,
...
facts carry no \env{type} or \env{difficulty}, so \file{find-facts} has two
filters where \file{find-problems} has four. Everything below holds for
both.
...
\file{pick-problems} (and \file{pick-facts}, identically) threads those two
...
(\texttt{<Space>ip}) and \texttt{:ImportFact} (\texttt{<Space>if}) are the same
