...
The output mirror is one \file{.OUTDIR} file, found by walking up from the
...
either order. A nearer \file{.OUTDIR} in a subdirectory wins, letting a
subtree mirror somewhere else. With no \file{.OUTDIR} above it at all, the PDF
lands next to its source.

\file{SAMPLE-OUTDIR}, shipped with the machinery, is the commented template --
copy it to \file{.OUTDIR} and uncomment. \file{.OUTDIR} is meant to be
...
\file{.OUTDIR} and tag the view with a matching \texttt{:LABEL} suffix in the
magic comment:
...
the git-ignored \file{.OUTDIR} maps it to a path, no machine-specific path ever
