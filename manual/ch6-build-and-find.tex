...
$ find-facts    [--topic PAT] [--source PAT] [--untagged] [--vocab]
                [--import] [--tsv] [PATH ...]
$ find-passages [--untagged] [--vocab] [--import] [--tsv] [PATH ...]
\end{manexsource}
...
the filter set -- facts carry no \env{type} or \env{difficulty}, and a passage
bank is not searched at all, so the three have four filters, two, and none. Everything below holds for all three.
...
keys, because both options \cs{importpassage} takes belong to the importing
document rather than to the passage: \env{level}, where the passage's natural
depth lands, and \env{note}, a gloss printed under its heading in that document
alone (section~\ref{sec:ref-passagemeta}). Emitted passage lines are
