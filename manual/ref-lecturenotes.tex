...
MathStuff, TitleBlock, Hyperlinks, CourseBoxes and InstructorNotes -- the full
stack -- plus \file{enumitem} and \file{tabularray}, the house list and table
packages, so a problem file that uses either compiles here as it does on an
assessment. Workflow: section~\ref{sec:lectures}.
...
\cs{NotesProblemLabelWord} & the noun imported problems render as
  ...
  \file{.course-machinery-local/textbook.tex}. Routed into \cs{ProblemLabelWord} so a
  shared text layer cannot disturb Assessment's fixed ``Q''.\\
\cs{lecturetoc} & the lecture's title and table of contents, then a page
  break. The title (``1.2\quad Basic Classes of Functions'', with a rule
  under it) is read from \cs{booksection} or \cs{lecturetitle}; there is no
  ``Contents'' heading. Document-called, not class-emitted: a short lecture
  wants no list, and only you know which those are -- but such a lecture
  gets no printed title either, only the running head. Put it straight
  after \cs{booksection}. See ``The contents list'' below.\\
\cs{LectureTitleFont} & the title's face (default
  \cs{sffamily}\cs{bfseries}\cs{LARGE}).\\
...
\begin{itemize}
  \item \textbf{Indented under the headings.} Section and subsection
    entries are full-size sans bold with space above; sub-subsections (sans)
    and results (serif) sit one indent in, and a result too long for one line
    wraps with a hanging indent, unhyphenated, keeping its page number on the
    last line. This replaced (2026-09-24) a flat list distinguished by
    typeface alone, in which the headings were \emph{smaller} than the
    results beneath them. A sectionless lecture's list simply sits one indent
    in: the layout stays unconditional.
  \item \textbf{Page numbers on the handout only,} in a right-aligned
    column. The projector copy prints
