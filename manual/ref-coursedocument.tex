...
with old-style body figures; \texttt{[lining]} opts out), TitleBlock,
Hyperlinks, InstructorNotes, \file{enumitem} (tunable lists --- a policy
document is more list than prose) and \file{tabularray} (the same reasoning
for tables: grade scales, rosters, a term schedule); no
...
mahogany -- and it is the colour that prints: headings, the title block
and internal links all carry it unmodified, so restyling a course
document is a one-line \cs{colorlet}. \env{HeadingColor} and
\env{TitleColour} are aliases of it rather than shades of it; they exist
only because titlesec, TitleBlock and hyperref each want a colour \emph{name}
to expand at use. The identity is deliberately not green: green means
\emph{solutions / not for students} across the toolkit
(\env{SolutionColor}), and a student-facing document must not read as
green.

If a restyle ever does want a darker heading, mix with lowercase
\env{black} -- xcolor's grey, which darkens in RGB and holds the hue.
Capital \env{Black} is dvipsnames cmyk $(0,0,0,1)$: it mixes in CMYK and
raises $K$, greying the colour as it darkens.
...
TitleBlock hooks are set to colour the title and stamp it \emph{at (time)} --
