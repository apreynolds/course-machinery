...
the bank with points attached. The skeleton, from the demo repo's
\file{_assessment/midterm.tex}:
...
\importproblem[points=5]
  {../differentiation/problems/}{chain-rule-basic.tex}
\workspace[5]
...
\end{document}
\end{manexsource}
...
render with the neutral ``Problem $N$.''\ style); build the demo midterm to
see the real thing.
...
\begin{problemgroup}{Evaluate}
  \importproblem[points=5, hint]
    {../differentiation/problems/}{find-derivative.tex}
  \workspace[3]
  \importproblem[5]
    {../differentiation/problems/}{find-derivative2.tex}
  \workspace[2]
\end{problemgroup}
\end{manexsource}
...
\begin{problemchoice}[select=2, points=5]
  \importproblem[]
    {../differentiation/problems/}{mc-graph-grid.tex}
  \FreshPage
  \importproblem[]
    {../differentiation/problems/}{multiple-choice-example.tex}
  \FreshPage
  \importproblem[]
    {../differentiation/problems/}{multiple-choice-example2.tex}
\end{problemchoice}
\end{manexsource}
...
With \texttt{\%! views: solutions, hints, instructor} in the magic comment,
one midterm source builds four PDFs: the student paper, an answer key
(\file{-SOLUTIONS}), a hints copy (\file{-HINTS}), and the staff bundle
(\file{-INSTRUCTOR}: solutions + hints + instructor notes, inside the
page-edge \emph{Instructor copy} frame). Under the default overlay mode all
four have \emph{identical page breaks} -- the whole point of
section~\ref{sec:overlay}. Build mechanics are in
