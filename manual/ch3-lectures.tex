...
\begin{itemize}
  ...
  \item \cs{importfact} (same shape again: \texttt{[number, desc]} then
    directory and file) drops in a snippet from the \emph{fact} bank
    (section~\ref{sec:fact-files}) -- a definition or rule stated once in
    \file{facts/} and quoted wherever a lecture or formula sheet wants it.
    The fact renders bare unless the document opts in to showing its
    provenance via the \cs{ShowFactMeta} hook
    (section~\ref{sec:ref-factmeta}).
\end{itemize}
