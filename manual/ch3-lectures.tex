...
% The text layer: what this course's book names things, where it lives.
...
whole story. And the text layer \file{.textbook/textbook.tex} is \cs{input}
\emph{in the preamble} (its box declarations must run before the document
starts), via a relative path from the repo root -- section~\ref{sec:extending}
explains what belongs in that file, and section~\ref{sec:ref-textref} what its
section map buys you.
...
Two macros carry the outside world into a lecture, and both exist so that a
link is written \emph{once}, consistently, and can be checked later:
...
\cs{textbookref} resolves through the section map in your text layer
(\file{.textbook/textbook.tex}) and links to the section's page --- keyed on
the section, not the finer coordinate, because page URLs are the stable part
of a web text and in-page anchors are what rots. If the section is not
declared, the label still prints, unlinked, and the build tells you at the
end. Full details, including where \cs{textbookref} may and may not appear
(never inside a shared \file{problems/} or \file{facts/} file), are in
