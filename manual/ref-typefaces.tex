\section{Typefaces.sty}\label{sec:ref-typefaces}

The typeface stack, chosen once for every course document: Libertinus
serif (semibold for bold), Merriweather sans (scaled to sit between the
serifs' lowercase and uppercase heights), \file{newtxtt} mono, and
newtxmath in its \texttt{libertinus} dress -- plus \file{mathalpha} for
distinct $\mathcal{C}$/$\mathscr{S}$/$\mathfrak{f}$/$\mathbb{B}$
alphabets, \file{microtype}, and \file{bm}.

This package \emph{owns the load-order contract}: it loads amsmath first,
then newtxmath with its \texttt{amsthm} option -- the one point in
newtxmath's load where amsthm can arrive in the right order. Classes and
packages must therefore never load amsmath or amsthm themselves, and
MathStuff, CourseBoxes and InstructorNotes all require Typefaces before
them (each enforces it with a \cs{PackageError}). The full story:
section~\ref{sec:load-order}.

\subsection*{Options}

\begin{center}
\begin{tabularx}{\linewidth}{@{}l Y@{}}
\texttt{osf} & old-style figures in serif body text (CourseDocument
  passes it by default -- prose documents).\\
\texttt{lining} & the default, spelt out (LectureNotes, Assessment --
  figures are information). CourseDocument passes this one instead when
  given its own \texttt{[lining]} option
  (section~\ref{sec:ref-coursedocument}).\\
\end{tabularx}
\end{center}

\subsection*{The four-knob figure model}

Figures appear in four places; each has exactly one independent control,
and only the first varies by document type:

\begin{center}
\begin{tabularx}{\linewidth}{@{}l Y@{}}
serif body prose & the \texttt{[osf]} option -- \emph{the only knob}.\\
sans (headings) & pinned lining, always.\\
math & always lining (newtxmath).\\
tabular contents & always lining, via an every-\env{tabular} hook.\\
\end{tabularx}
\end{center}

So under \texttt{[osf]}, running prose gets old-style figures while course
numbers, math and tables stay lining. The sans row is independent of the
knob because \cs{useosf} cannot reach a sans that Libertinus did not load.
Local overrides: \cs{liningnums}\texttt{\{2026\}} /
\cs{oldstylenums}\texttt{\{2026\}} -- and \cs{course} is built on the first
of those, so its numerals are pinned outright rather than by borrowing the
sans.

Two caveats on that fourth row. The hook is on \env{tabular} itself, so a
table built by another package -- \env{longtblr} and the rest of
\file{tabularray}, which is not built on \env{tabular} -- does \emph{not}
inherit it, and a schedule table full of dates will stay osf under
\texttt{[osf]}. No lecture or assessment uses \texttt{[osf]}, so the gap is
only live in CourseDocument; the fix, when it is needed, is a \file{tabularray}
counterpart of the hook in \file{Typefaces.sty}, noted there. And when a whole document reads better in lining figures,
the first row is a per-document choice, not a fixed property of the class:
CourseDocument's \texttt{[lining]} option
(section~\ref{sec:ref-coursedocument}) is the switch, because there is no
global undo to reach for -- Libertinus defines \cs{useosf} and no
\cs{uselining} twin.

\subsection*{Commands and lengths}

\begin{center}
\begin{tabularx}{\linewidth}{@{}l Y@{}}
\cs{course}\texttt{[MATH]\{1013\}} & the course designation in the body
  serif, joined by a tie so it cannot break across lines; the optional
  prefix defaults to MATH. \cs{liningnums} on the number is load-bearing:
  it is what keeps a course number lining under \texttt{[osf]}, and the
  only reason the macro exists rather than typing the designation out.\\
\cs{alphabet} & the measured width of the lowercase alphabet in the real
  body font. Classes set \texttt{textwidth} $= 2.6$\cs{alphabet} ($\approx
  66$ characters, the classic readable measure) -- a \emph{rule} that
  re-derives itself when the font or size changes, where a frozen length
  would silently stop tracking.\\
\end{tabularx}
\end{center}
