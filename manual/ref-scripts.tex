...
wrapper & a throwaway \texttt{article} document in \env{\$TMPDIR}, never
  written into your tree. It loads \file{Exercises.sty} directly, because none
...
courses, living in the \file{teaching/} submodule --- is the one bank with
...
(\texttt{find-passages teaching/passages}). Run bare from a course root, with
no block to tell a passage from any other \file{.tex}, it would report every
lecture and assessment as a passage --- so it refuses instead of defaulting to
the current directory the way the other four do.
