...
The passage bank (section~\ref{sec:ref-passagemeta}) --- prose shared across
courses, living in the \file{teaching/} submodule --- is searched the same way
again. \file{find-passages} and \file{pick-passages} read \cs{PassageMeta} blocks
and emit \cs{importpassage} lines. There are \emph{no} filters --- a passage's
filename is its title and the bank is small, so it is listed rather than searched
--- and \texttt{-{}-tsv} columns are (path, title, requires). The Vim command
\texttt{:ImportPassage} (\texttt{<Space>ig}) mirrors the other two.

Two ways this pair differs from the other four. It takes \emph{no}
\texttt{-{}-with}: a passage has no injectable keys, since the two options
\cs{importpassage} accepts --- \env{level} and \env{note} --- both belong to the
importing document. And because the bank is a sibling submodule rather than a directory of
the course, give it as an operand (\texttt{find-passages teaching/passages});
run bare from a course root it would walk the course as well and report every
untagged file in it.
