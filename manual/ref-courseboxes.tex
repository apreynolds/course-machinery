...
\env{yourturn} (blue, pen glyph), \env{prepwork} (blue, book glyph),
\env{notcovered} (blue, forward glyph), and \env{tip} (orange), which takes
an optional audience tier --- \texttt{[surviving]} or \texttt{[thriving]} ---
appending ``for the \emph{surviving/thriving}'' to the head.

\env{yourturn} is the one aside whose head is also used \emph{outside} a box:
\cs{importproblem}'s \texttt{yourturn} key prefixes a marked example's heading
with it (section~\ref{sec:lectures}). So the head lives in a macro,
\cs{YourTurnMark} --- glyph, words and colour, but no colon, that being head
\emph{punctuation} and hence the style's (\texttt{cbnotecolon}). Its contents-list
twin, \cs{YourTurnTocMark}, is the bare glyph wrapped in \cs{texorpdfstring},
because the same entry also becomes a PDF bookmark, where a font glyph cannot be
represented. Change the wording in those two places and both sites follow.
...
\file{.course-machinery-local/textbook.tex}, with the factories below
...
Both factories \textbf{refuse a name that is already a control sequence},
with an error naming the fix rather than LaTeX's bare ``Command
\cs{rule} already defined''. This is not a rare trap: \env{rule},
\env{proof}, \env{box}, \env{list}, \env{table}, \env{figure}, \env{part},
\env{item}, \env{index}, \env{title}, \env{center}, \env{quote} and
\env{abstract} are all taken in one of these documents, and \env{problem} is
\file{Exercises.sty}'s own --- so the guard protects the exercise machinery
as well as the boxes. The escape is that the \emph{display} name is a
separate argument: capitalising the environment usually reads best, because
it then matches what prints ---
\texttt{\char`\\NewTextResult\{Rule\}\{Rule\}\{cb/textresult\}}.

A name \emph{this package already declared} gets a different message, since
that is a duplicate declaration rather than a collision --- nearly always a
text layer re-declaring something CourseBoxes already ships.

Unlike \cs{importfact}'s \texttt{kind=} guard, which warns and imports the
fact bare, this one is an \textbf{error}: there is no sensible fallback.
Skipping the declaration silently would leave every later
\texttt{\char`\\begin\{rule\}} running LaTeX's \cs{rule} instead.
