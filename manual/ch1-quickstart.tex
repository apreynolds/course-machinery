...
a hidden subdirectory. The demo course (\file{math101-demo}) is the
reference layout:
...
Cross-file inclusion uses the \file{import} package's relative paths:
a lecture pulls \file{../problems/find-derivative.tex}, the midterm pulls
\file{../differentiation/problems/}\allowbreak\file{find-derivative.tex} --
the \emph{same} file, and any images that file loads resolve against the
\emph{file's own} directory, so one source serves every consumer.
