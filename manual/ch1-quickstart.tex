...
course-demo/
  ...
  .latexmkrc           puts the machinery and .textbook on TEXINPUTS
  .MIRRORDIR           optional PDF mirror destination
  .textbook/           the text layer: what a book names things,
    textbook.tex       and where it lives online. This one is the
    openstax.tex       course default; a second file is a second book,
                       chosen per lecture with \usetext
...
The one course-wide content file is \file{.textbook/textbook.tex}, the
\emph{text layer}: everything that depends on which book the course is
taught from -- the named result boxes it uses (APEX's ``Key Idea''), the
noun lectures call an example, and the base URL the book lives at.
Masters do not \cs{input} it --- it is found by name and loaded
automatically, so a lecture may sit at any depth and a new master cannot
forget it. Section~\ref{sec:extending} explains the one-question test for
what belongs there, and why it is guarded. Which \emph{section} of the book a
given lecture covers is declared in that lecture, not here
