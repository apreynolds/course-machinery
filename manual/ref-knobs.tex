...
\file{.course-machinery-local/textbook.tex} and travels with the book; if no (your
...
\cs{NotesProblemLabelWord} & LectureNotes & the notes noun -- per
  \textbf{text}, so its home is \file{.course-machinery-local/textbook.tex}.\\
...
\cs{BonusMarginLabel}, \cs{BonusMarginRule} & Assessment & bonus wording
  / divider.\\[0.5ex]
\cs{TitleColor}, \cs{TitleStamp}, \cs{TitleMarker} & TitleBlock & title
  styling hooks.\\
\cs{LinkColor}, \cs{UrlColor} & Hyperlinks & link colours.\\
\cs{SecRefFormat} & Hyperlinks & how \cs{secref} words a
  cross-reference (default ``the \emph{title} section'').\\
...
\cs{TextRefBase} & TextRef & the textbook base URL -- set via
  \cs{DeclareTextBase} (or \cs{SetTextBase}) in the \textbf{text} layer,
  not renewed here.\\
\cs{BookSectionSeparator} & TextRef & between number and title in a
  lecture's head (default ``\texttt{:\ }'').\\
\cs{BookTitleStyle} & TextRef & how \cs{booktitle} sets a book's title
  (default \cs{emph}). House style, not a fact about any book, so it
  stays here and not in the registry.\\
\cs{CourseHeadRule}, \cs{CourseRunningHead} & TitleBlock & the running
  head's rule width and its whole rendering.\\[0.5ex]
...
\env{CourseIdentity} (\cs{colorlet}) & CourseDocument & the course
  identity colour -- headings, title and internal links, unmodified.\\
